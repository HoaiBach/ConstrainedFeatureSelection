...
def bonferroni_dunn_test(ranks, control=None):
    ...
    p_values, z_values, comparisons = list(map(list, list(zip(*sorted(zip(p_values, z_values, comparisons), key=lambda t: t[0])))))
...
def nemenyi_multitest(ranks):
    ...
    values = list(ranks.values())
    keys = list(ranks.keys())
    versus = list(it.combinations(list(range(k)), 2))
...
def _S(k):
    ...
    if k == 0 or k == 1:
        ...
        result = set()
        for j in reversed(list(range(1, k+1))):
            tmp = S(k - j)
            for s in tmp:
